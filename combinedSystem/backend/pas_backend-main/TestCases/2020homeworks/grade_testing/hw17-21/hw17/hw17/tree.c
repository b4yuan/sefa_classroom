...
/*
  NOTE: This function print the path based on the tree
*/
void printPath(TreeNode * tr, char* path,int num)
{
  if(tr->value != -1) // otherwise would be nothing 
  { 
    printf("%c %d ",tr->value,tr->value);
    for(int j=0;j<num;j++) printf("%c",path[j]);
    printf("\n");
  }
  ...
  if(tr ->left != NULL){
    path[num] = '0';
    printPath(tr->left,path,num+1);
  }

  if(tr ->left != NULL){
    path[num] = '1';
    printPath(tr->right,path,num+1);
  }
