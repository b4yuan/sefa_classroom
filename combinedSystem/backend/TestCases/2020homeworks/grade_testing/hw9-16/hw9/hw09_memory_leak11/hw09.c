...
bool readData(char * filename, int * * arr, int * size)
{
  ...
  *arr = malloc(sizeof(int)*(*size)); 
  ...
  int numRead = 0;
  numRead = fread(arr[0],sizeof(int),*size,fptr);
  if (numRead != *size){
    free(*arr);
    fclose(fptr);
    return false;
...
bool writeData(char * filename, const int * arr, int size)
{
  ...
  int numElem = fwrite(&arr[0],sizeof(int),size,fptr);
...
// a static function is invisible to other files
{
  ...
  int *arr1,*arr2,*arr3;
  arr1 = malloc(sizeof(int)*(m-l+1+1));
  arr2 = malloc(sizeof(int)*(r-m+1));
  arr3 = malloc(sizeof(int)*(r-l+1+1));
  ...
  int x=l,y=m+1,z=0;
  while(x<m+1 && y<=r && z<r-l){
    if(arr1[x-l]< arr2[y-m-1]){
      arr3[z++] = arr1[x++-l];
    }
    else{
      arr3[z++] = arr2[y++-m-1];
